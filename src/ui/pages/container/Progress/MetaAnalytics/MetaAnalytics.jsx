...
export default function MetaAnalytics(props) {
  ...
  const metaAnalyticsDataJson = useSelector(
    (state) => state.getMetaAnalyticsData.originalData
  );
  ...
  const getMetaAnalyticsdata = () => {
    setLoading(true);
    const userObj = new MetaAnalyticsDataAPI(
    ...
    metaAnalyticsData.forEach((dataArray, index) => {
      if (dataArray.length) {
        ...
        dataArray.forEach((data) => {
          ...
          doc.text(
            `Ann Cumulative word Count: ${
              data.annotation_cumulative_word_count || "N/A"
            }`,
            10,
            yOffset + 5
          );
          doc.text(
            `Rew Cumulative word Count: ${
              data.review_cumulative_word_count || "N/A"
            }`,
            10,
            yOffset + 10
          );
