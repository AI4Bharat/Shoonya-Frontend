...
const features = [
  ...
  {
    img: UsefulUI,
    title: "Clean & Powerful UI",
    description:
      "The heavy lifting is done in the background. The UI is designed to be as simple as possible, but not compromising on any of the features. Easy to submit the documents for digitization, translation, review, workflow management, job monitoring etc.",
  },
  {
    img: RobustIntegrations,
  ...
  },
  {
    ...
    title: "Easy to Use",
    description:
      "Anyone with basic knowledge in language can be onboarded to Chitralekha. It does not limit the scope of your tasks; you can use it to build pipelines to generate the final translated version of the submitted documents.",
  },
  {
    img: OpenSource,
    title: "Open Source",
...
const Features = () => {
  ...
  return (
    <ThemeProvider theme={introTheme}>
      <Grid container justifyContent="center" sx={{ mt: 5, px: 4 }}>
        <Typography
          variant="h4"
          sx={{ fontSize: "55px", lineHeight: 1, color: "#51504f", mb: 5 }}
        >
          Features
        </Typography>
      </Grid>
      <Grid container spacing={4} justifyContent="center" sx={{ px: 4 }}>
        {features.map((feature, index) => (
          <Grid item xs={12} sm={6} md={6} key={index} textAlign="center">
            <img src={feature.img} alt={feature.title} className={classes.featuresimg} />
            <Typography variant="h5" sx={{ mt: 2, mb: 1 }} className={classes.featuresTitle}>
              {feature.title}
            </Typography>
            <Typography
              variant="body1"
              sx={{ textAlign: "justify", fontSize: "16px", color: "#707070", lineHeight: "25px" }}
            >
              {feature.description}
            </Typography>
          </Grid>
