...
const validateL1Format = (subtitles) => {
  const errors = [];
  ...
  subtitles.forEach((subtitle, index) => {
    ...
    // Check 1: Text must start with {
    if (!text.startsWith('{')) {
      errors.push(`Segment ${index + 1}: Text must start with '{'`);
      return;
    }
    
    // Check 2: Text must end with }
    if (!text.endsWith('}')) {
      errors.push(`Segment ${index + 1}: Text must end with '}'`);
      return;
    }
    
    // Remove outermost braces for parsing
    const innerText = text.slice(1, -1);
    
    // Split by spaces but preserve spaces inside braces
    const segments = [];
    let currentSegment = '';
    let braceCount = 0;
    ...
    for (let i = 0; i < innerText.length; i++) {
      ...
      if (char === '{') {
        ...
        // Space outside braces - separator
    ...
    // Check 3: Each segment must be wrapped in {}
    segments.forEach((segment, segIndex) => {
      if (!segment.startsWith('{') || !segment.endsWith('}')) {
        errors.push(`Segment ${index + 1}, part ${segIndex + 1}: '${segment}' must be wrapped in {}`);
      } else {
        // Check 4: Inside braces, only allow letters, numbers, spaces, |, and noise tags
        const inner = segment.slice(1, -1);
        if (!/^[a-zA-Z0-9\s|<>\-]+$/.test(inner.replace(/<[^>]+>/g, ''))) {
          errors.push(`Segment ${index + 1}, part ${segIndex + 1}: Contains invalid characters`);
        }
      }
    });
  });
