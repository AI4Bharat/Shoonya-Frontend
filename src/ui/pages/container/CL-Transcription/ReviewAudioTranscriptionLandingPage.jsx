...
import React, {
  ...
  const AudioTranscriptionLandingPage = () => {
    ...
    useEffect(() => {    
  if( AnnotationsTaskDetails.some((obj) => obj.result.every((item) => Object.keys(item).length === 0))){
    const filteredArray = AnnotationsTaskDetails.filter((obj) => obj?.result.some((item) => Object.keys(item).length > 0));
   ...
   dispatch(setSubtitles(sub, C.SUBTITLES));
    ...
    useEffect(() => {
     ...
     console.log( localStorage.getItem("Stage") === "review","StageStageStageStage")
    }, []);
    ...
    const tasksComplete = (id) => {
      if (id) {
        ...
        navigate(`/projects/${projectId}/ReviewAudioTranscriptionLandingPage/${id}`);
      } else {
        // navigate(-1);
        // resetNotes();
        setSnackbarInfo({
    ...
    const handleReviewClick = async (
      ...
      const PatchAPIdata = {
        ...
        lead_time:
         ...
         result,
         ...(value==="accepted" || value==="accepted_with_minor_changes" ||value==="accepted_with_major_changes") && {parent_annotation:parentannotation},
      }
      ...
      if (res.ok) {
        ...
        if (value === "accepted"||value === "accepted_with_minor_changes"||value === "accepted_with_major_changes") {
            setSnackbarInfo({
              open: true,
  ...
  <Button
              ...
              variant="contained"
              color={
                annotationNotesValue !== "" &&annotationNotesValue !== null ||
                (superCheckerNotesValue !== "" && superCheckerNotesValue !== null)
                  ? "success"
                  : "primary"
              }
            ...
            >
              Notes{" "}
              {annotationNotesValue !== "" && annotationNotesValue !== null ||
                (superCheckerNotesValue !== null && superCheckerNotesValue !== "" && "*")}
            </Button>
          ...
          >
            ...
            <TextField
              ...
              onChange={event=>setsuperCheckerNotesValue(event.target.value)}
