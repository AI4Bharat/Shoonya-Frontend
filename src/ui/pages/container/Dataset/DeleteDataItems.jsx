...
import {
    ...
    Box,
    TextField,
    Grid, Typography, Radio,Dialog, DialogActions, DialogContent, DialogContentText,Checkbox,IconButton
} from "@mui/material";
...
export default function DeleteDataItems(permissionList) {
 ...
 const viewPermissions = permissionList?.permission?.can_view_delete_data_item || [];
 const usePermissions = permissionList?.permission?.can_use_delete_data_item || [];
  ...
  use: usePermissions
    ...
    useEffect(() => {
        if (permissionList && permissionList?.permission) {
            setview(permissionList?.permission?.can_view_delete_data_item);
            setuse(permissionList?.permission?.can_use_delete_data_item);
            const viewPermissions = permissionList?.permission?.can_view_delete_data_item || [];
        const usePermissions = permissionList?.permission?.can_use_delete_data_item || [];
        setSelectedOptions({
          view: viewPermissions,
    ...
    // console.log(permissionList?.permission?.can_view_delete_data_item,selectedOptions.view);
    ...
    return (
        <div >
            ...
            <Button
                sx={{ width: "150px" , borderRadius: loggedInUserData?.role === 6? "8px 0 0 8px" : 3,}}
                aria-describedby={id}
                variant="contained"
                color="error"
                onClick={canUseDownloadButton(loggedInUserData?.role) ? handleClick : null}
                disabled={
                    Dataitems.length < 0 && ( canViewDownloadButton(loggedInUserData?.role))&& loggedInUserData?.role!==6
                      ? true
                      : false
                  }
                >
