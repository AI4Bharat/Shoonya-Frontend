...
const Projects = () => {
    ...
    const isReviewer = ProjectDetails?.enable_task_reviews && (roles.filter((role) => role.role === userDetails?.role)[0]?.isReviewer || ProjectDetails?.annotation_reviewers?.some((reviewer) => reviewer.id === userDetails?.id));
    useEffect(() => {
