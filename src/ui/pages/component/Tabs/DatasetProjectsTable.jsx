...
export default function DatasetProjectsTable({ datasetId }) {
  ...
  const datasetProjects = useSelector((state) => state.getDatasetProjects.data);
  ...
  const getExportProjectButton = async (project) => {
    setLoading(true);
    const projectObj =
      project.project_type === "ConversationTranslation"
        ? new GetExportProjectButtonAPI(project.id, project.dataset_id[0])
        : new GetExportProjectButtonAPI(project.id);
  ...
  const pageSearch = () => {
    return datasetProjects.filter((el) => {
      if (SearchWorkspaceMembers == "") {
        ...
        el.title?.toLowerCase().includes(SearchWorkspaceMembers?.toLowerCase())
      ) {
        return el;
      }
      //   } else if (
  ...
  const data = datasetProjects
    ? pageSearch().map((project) => ({
        ...project,
        actions: () => (
          <Stack direction="row" spacing={2}>
            ...
            {userRole.Admin === loggedInUserData?.role ? (
              <CustomButton
                sx={{ borderRadius: 2, height: 37 }}
                onClick={() => getExportProjectButton(project)}
                label="Export"
              />
            ) : null}{" "}
            <CustomButton
              sx={{ borderRadius: 2 }}
              onClick={() => getPullNewDataAPI(project)}
              label="Pull New Data Items"
            />
          </Stack>
        ),
      }))
