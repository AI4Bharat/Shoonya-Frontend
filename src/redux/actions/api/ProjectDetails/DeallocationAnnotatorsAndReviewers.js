...
export class DeallocateTaskById extends API {
  constructor(projectId, taskId, selectedUser, timeout = 2000) {
    ...
    if(selectedUser === "annotation") {
      this.endpoint = `${super.apiEndPointAuto()}/${ENDPOINTS.getProjects}${projectId}/unassign_tasks/`;
    }
    else if(selectedUser === "superchecker") {
      this.endpoint = `${super.apiEndPointAuto()}${ENDPOINTS.getProjects}${projectId}/unassign_supercheck_task/?task_id=${taskId}`;
    }
  }
  ...
  constructor(
    ...
    const queryString =
      radiobutton === "annotation"
        ? `unassign_tasks/?annotator_id=${annotatorsUser}&annotation_status=["${annotationStatus}"]`
        : radiobutton === "review"
        ? `unassign_review_tasks/?reviewer_id=${reviewerssUser}&review_status=["${reviewStatus}"]`
