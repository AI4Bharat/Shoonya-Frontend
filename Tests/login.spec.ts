...
test("Login Test", async ({ page }) => {
  ...
  await page
    ...
    .fill(process.env.NEXT_PUBLIC_TEST_EMAIL || "");
  await page
...
test("Reset Password Test", async ({ page, request }) => {
  ...
  // Fill in email
  await page.locator("input").fill(process.env.NEXT_PUBLIC_TEST_EMAIL || "");
  ...
  // expect(requestBody).toEqual({ email: process.env.NEXT_PUBLIC_TEST_EMAIL });
